...
class EvaluacionGeneralController extends Controller
{
        ...
        try {
            ...
            $evaluacionesFormateadas = $evaluaciones->map(function ($ev) {
                return [
                    ...
                    'polinizador' => [
                        'id' => $ev->polinizador ? $ev->polinizador->id : null,
                        'nombre' => $ev->polinizador ? $ev->polinizador->nombre : 'N/A'
                    ],
            ...
            $evaluacionesPorOperario = [];
            foreach ($evaluacionesFormateadas as $ev) {
                $operarioId = $ev['polinizador']['id'] ?? 'sin_id';
                $operarioNombre = $ev['polinizador']['nombre'] ?? 'N/A';
                $key = $operarioId . '|' . $operarioNombre;
                
                if (!isset($evaluacionesPorOperario[$key])) {
                    $evaluacionesPorOperario[$key] = [];
                }
                $evaluacionesPorOperario[$key][] = $ev;
            }
    ...
    public function getByOperario(Request $request, string $operarioId)
    {
        try {
            ...
            // O si tiene acceso total o solo a su finca (puede ver operarios de su finca)
            ...
            $query = EvaluacionGeneral::with([
                'evaluador', 
                'polinizador', 
                'lote.finca', // Cargar finca a través del lote
                'evaluacionesPolinizacion' // Cargar las sub-evaluaciones
            ])
            ->where('idpolinizadorev', $operarioId);
            
            // Si es evaluador con acceso solo a su finca, filtrar por su finca
            if ($usuario->soloAccesoAFinca()) {
                $query->whereHas('lote', function($q) use ($usuario) {
                    $q->where('idFinca', $usuario->idFinca);
                });
            ...
            // Formatear la respuesta para incluir los datos necesarios
            $evaluacionesFormateadas = $evaluaciones->map(function ($ev) {
                return [
                    'id' => $ev->id,
                    'fecha' => $ev->fecha, 
                    'hora' => substr($ev->hora, 0, 5),
                    'semana' => $ev->semana,
                    'evaluador' => $ev->evaluador ? ['id' => $ev->evaluador->id, 'nombre' => $ev->evaluador->nombre] : null,
                    'polinizador' => $ev->polinizador ? ['id' => $ev->polinizador->id, 'nombre' => $ev->polinizador->nombre] : null,
                    'lote' => $ev->lote ? ['id' => $ev->lote->id, 'descripcion' => $ev->lote->descripcion] : null,
                    'finca' => $ev->lote && $ev->lote->finca ? ['id' => $ev->lote->finca->id, 'descripcion' => $ev->lote->finca->descripcion] : null,
                    'seccion' => $ev->getAttribute('seccion') ?? ($ev->evaluacionesPolinizacion->first()->seccion ?? 'N/A'),
            ...
            // Devolver el array formateado
            return response()->json($evaluacionesFormateadas, 200); 
        } catch (\Exception $e) {
            return response()->json([
                'error' => 'Error al obtener las evaluaciones generales por operario: ' . $e->getMessage(),
                'linea' => $e->getLine(),
                'archivo' => $e->getFile()
            ], 500);
        }
